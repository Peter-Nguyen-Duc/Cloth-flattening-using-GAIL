...
class URSim_SKRL_env(MujocoEnv):
    ...
    def __init__(self, args, name, render_mode="human"):
        ...
        observation_space_size = 2 * self.actuated_joints + 2
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # insert into observation which tasks that has been completed
        task_completion = np.array([0,0])

        if abs(joint[5] - self.desired_rotated_joint_vals[5]) < 0.05:
            task_completion[0] = 1


        observation = np.concatenate((observation, task_completion))
    ...
    def get_box_position(self, block_pos, block_rot):
        ...
        box_shift_translation_box_space = [-0.075,   0.2,   -0.075,  1.]
    ...
    def step(
        ...
        a = self.desired_rotated_joint_vals
    ...
    def reset_model(
        ...
        block_pos=[0.8, 0.37537452, 0.2] 
        block_rot=[0.5, -0.5, -0.5, 0.5]


        rand_rot_max = np.pi*2

        # Random rotation of the block
        eul_rot = [0,np.random.random()*rand_rot_max,0] 
        ...
        noise_max_pos = 0 # np.pi/64 # Remove noise for debug
        ...
        align_box_position, align_box_rotation = self.get_box_position(block_pos=block_pos, block_rot=block_rot)
        

        # Make robot rotation align to block
        robot_rot_quat = [-0.19858483999999996, -0.00311175, 0.0012299899999999998, 0.98007799]
        robot_align_with_block = qqmul(qconj(robot_rot_quat), block_rot)
        rotation_block_2_robot = [np.pi/2, np.pi/2, -np.pi/2]
        robot_desired_aligned_rotation = qqmul(robot_align_with_block, r2q(eul2r(rotation_block_2_robot)))



        angle_dif = qqmul(robot_desired_aligned_rotation, qconj(align_box_rotation))

        print("angle dif: ", angle_dif)
        align_box_pose = make_tf(align_box_position, align_box_rotation)


        joint_vals = self.ur5e.ik(align_box_pose)[0]

        self.desired_rotated_joint_vals = copy.deepcopy(joint_vals)

        if self.desired_rotated_joint_vals[5] + angle_dif > np.pi*2:
            self.desired_rotated_joint_vals[5] += angle_dif - np.pi*2
        else:
            self.desired_rotated_joint_vals[5] += angle_dif

        print("angle diff: ", angle_dif)
        print("joint 5: ", self.desired_rotated_joint_vals[5])
    ...
    def _plot_robot_data(self):
        ...
        ax.set_ylim(min(min_val, -1)-0.1, max(max_val, 1) + 0.1)
