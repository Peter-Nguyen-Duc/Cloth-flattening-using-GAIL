...
def step_mujoco_simulation_MJX(data, model, model_mjx, ctrl, n_frames):
    """
    Step over the MuJoCo simulation.
    """
    print("entered MJX step")

    data.ctrl[:] = ctrl


    print("making mjx data")
    mjx_data = mjx.put_data(model, data)

    print("making mjx step")
    mjx.step(model_mjx, mjx_data)
...
class URSim_SKRL_env(MujocoEnv):
    ...
    def __init__(self, args, name, render_mode="human"):
        ...
        print("initialize mjx model")
        self.model_mjx = mjx.put_model(self.model)



        # Load expert data [AIRL]
        # path_expert_data_1 = os.path.abspath("learning/AIRL/expert_demo/C1_expert_data_gello_500/expert_memory.pkl")
    ...
    def step(
        ...
        step_mujoco_simulation_MJX(self.data, self.model,self.model_mjx, a, self.frame_skip)
