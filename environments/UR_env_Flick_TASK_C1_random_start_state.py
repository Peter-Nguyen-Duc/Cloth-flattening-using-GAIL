...
class URSim_SKRL_env(MujocoEnv):
    ...
    def __init__(self, args, name, render_mode="human"):
        ...
        # loading start_state_file

        self.start_state_folder = "learning/config_files/C1_start_state/random_inits/random_states_25-09-14_14-22-11"

        # get all pickle states in the start state folder.
        self.random_start_file_list = os.listdir(self.start_state_folder)



        # Load zero velocity state
        qvel_dir = os.path.abspath("learning/config_files/C1_start_state/zero_vel_config/zero_qvel_config.pkl")
        qvel_file = open(qvel_dir, "rb")
        self._zero_Q_vel_list = pickle.load(qvel_file)
    ...
    def _r(self) -> float:
        ...
        x_distance += positions[0][0] - positions[4][0]
        x_distance += positions[1][0] - positions[3][0]
        # Put it into a sigmoid to emphasize the sign error, and diminish the small errors

        sig_x_distance = 1 / (1 + np.exp(-x_distance*10)) # scale it to have a steeper curve

        # Only a problem when the z position is as desired (steady state)
        
        sig_x_distance *= max(0, 1 - z_position_error) # scale it to have a steeper curve
        ...
        orientation_error = orientation_error/6

        performance_metric = area_normalized - z_position_error_average - orientation_error - sig_x_distance
        ...
        area_normalized *= max(0, 1 - robot_x_error)
    ...
    def step(
        ...
        if self.step_number == 280:
            self.data_points += 1
            print(f"R ({self.data_points}): ", reward)

            address = "learning/reward_data/random_mass_0_1/random_mass_0_1.txt"  # Replace with your desired file path
    ...
    def reset_model(
        ...
        # chose random start state

        random_file = np.random.choice(self.random_start_file_list, 1)[0]
        random_file_dir = os.path.abspath(self.start_state_folder + "/" + random_file)
        qpos_file = open(random_file_dir, "rb")
        self.random_start = pickle.load(qpos_file)




        self.set_state(self.random_start, self._zero_Q_vel_list)
