...
from utils.mj import get_joint_pos, get_joint_names
...
def train(
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if d:
            ...
            # only take the last 5 evaluations for graphing
            if len(joint_position_list) > 5:
                joint_position_list = joint_position_list[-5:]
...
def eval(
    ...
    cum_reward = 0.0
    total_timesteps = 0
    # Debug value for saving the accelerations at all timesteps
    ...
    for i in range(args.eval_episodes):
        ...
        while not d:
            ...
            ddq_sub_list.append(robot.q[0])

            # Plot cartesian values... i think
            #cartesian_joint = get_joint_pos(robot._data, robot._model, "shoulder_lift")
            #cartesian_list.append(cartesian_joint)

            episode_timesteps += 1
            args.t = episode_timesteps
            # with timer():
            a = learning_algo.select_action(o, greedy=True).detach().cpu().numpy()
        ...
        ddq_list.append(np.array(ddq_sub_list))
    ...
    path = f"./data/{learning_algo.name}/{file_name}/"
    #Print Q values for both actions
    #/home/peter/OneDrive/Skrivebord/Uni stuff/Masters/Masters_project/mj_sim/data/Q_learning/ddq_joint_0.png



        # Save the plot
    plt.savefig(f"./data/{learning_algo.name}/{file_name}/ddq_joint_mean_and_variance.png")
    plt.close()

    # Plot ddq values for each episode from sublist
    fig, ax = plt.subplots()
    clrs = sns.color_palette("husl", 5)
    ax.plot(ddq_sub_list, c=clrs[0])
        # Save the plot
    plt.savefig(f"./data/{learning_algo.name}/{file_name}/Latest_value.png")
    plt.close()
